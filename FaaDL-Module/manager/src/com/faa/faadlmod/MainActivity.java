...
public class MainActivity extends Activity {
    ...
    private void maybeAskRoot() {
        final android.content.SharedPreferences sp =
                getSharedPreferences("fydl", MODE_PRIVATE);
        ...
        new android.app.AlertDialog.Builder(this)
                .setTitle("Akses root?")
                .setMessage("FYDL jalan penuh dengan root: eksekusi yang ditolak "
    ...
    private void extractBins() {
        ...
        // Prioritas modul Magisk: binary system (tanpa masalah exec/SELinux).
        // Wrapper dihitung valid dari keberadaannya (size kecil wajar).
        try {
            File sff = new File("/system/bin/faadl-ffmpeg");
            if (sff.exists()) {
                ffmpegFile = sff;
                log("ffmpeg: system modul");
            }
            File syt = new File("/system/bin/faadl-ytdlp");
            if (syt.exists() && syt.length() > 100000) {
                ytdlpFile = syt;
                log("yt-dlp: system modul");
            }
            StringBuilder sbl = new StringBuilder("sysbin=[");
            try {
                File[] sfs = new File("/system/bin").listFiles();
                if (sfs != null) {
                    boolean first = true;
                    for (int i = 0; i < sfs.length; i++) {
                        if (!sfs[i].getName().startsWith("faadl")) continue;
                        if (!first) sbl.append(",");
                        first = false;
                        sbl.append(sfs[i].getName());
                    }
                }
            } catch (Exception ignored) {}
            sbl.append("]");
            android.util.Log.d("FaaDL", sbl.toString());
        } catch (Exception e) {
            android.util.Log.d("FaaDL", "sysbin " + e.getMessage());
        }
    ...
    /** Binary modul (/system) dicek ulang tiap mau download (mount bisa telat). */
    private void preferSystemBins() {
        try {
            File sff = new File("/system/bin/faadl-ffmpeg");
            if (sff.exists()) {
                if (ffmpegFile == null
                        || !ffmpegFile.getAbsolutePath().equals(sff.getAbsolutePath())) {
                    ffmpegFile = sff;
                    log("ffmpeg: system modul");
                }
            }
            ...
            if (syt.exists()) {
                if (ytdlpFile == null
                        || !ytdlpFile.getAbsolutePath().equals(syt.getAbsolutePath())) {
                    ytdlpFile = syt;
                    log("yt-dlp: system modul");
                }
            }
        } catch (Exception e) {
            android.util.Log.d("FaaDL", "preferSys " + e.getMessage());
        }
    }
